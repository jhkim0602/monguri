...
// --- Types ---
type FeedbackType = "task" | "plan" | "question";
...
interface FeedbackItem {
  ...
  studentId: string;
  studentName: string;
  title: string; // Task Title or Goal or Question Summary
  subtitle?: string; // Subject or specific context
  date: Date;
  status: "pending" | "completed" | "reviewed" | "submitted";
  isUrgent?: boolean;
  data: any; // Raw data object for detail view
}
...
export default function FeedbackPage() {
  ...
  // --- Data Aggregation ---
  // 1. Tasks
  const taskItems: FeedbackItem[] = MENTOR_TASKS.filter(
    (t) => t.status === "submitted",
  ).map((t) => ({
    id: `task-${t.id}`,
    type: "task",
    studentId: "s1", // Mock
    studentName: "김멘티", // Mock
    title: t.title,
    subtitle: t.subject,
  ...
  // 2. Plan Reviews
  const planItems: FeedbackItem[] = PENDING_PLAN_REVIEWS.filter(
    (p) => p.status === "pending",
  ).map((p) => ({
    id: `plan-${p.id}`,
    type: "plan",
  ...
  // 3. Questions
  const questionItems: FeedbackItem[] = STUDENT_QUESTIONS.filter(
    (q) => !q.isAnswered,
  ).map((q) => ({
    id: `question-${q.id}`,
    type: "question",
    ...
    title: "학습 질문이 있습니다",
    subtitle: q.taskTitle,
    date: q.date,
    status: "pending", // Question pending answer
    isUrgent: true,
    data: q,
  }));

  const allItems = [...questionItems, ...planItems, ...taskItems].sort(
  ...
  const handleSendFeedback = () => {
    openModal({
      ...
      onConfirm: () => {
        ...
        // We do NOT clear feedbackText so they can keep editing if needed,
        // or we could clear it. User said "check it appearing", so keeping the state is safer.
        // But usually, one would clear the input. Let's keep the input for now as it syncs with the concept of "reviewing".
      },
    });
  };
          ...
          {filteredItems.length === 0 ? (
            ...
            filteredItems.map((item) => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <img
                    src={getStudentAvatar(item.studentName)}
