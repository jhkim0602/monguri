...
export default function DashboardPage() {
  ...
  const pendingFeedbacks = MENTOR_TASKS.filter(
    (t) => t.status === "submitted",
  ).length;
  const totalStudents = STUDENTS_MOCK.length;
      ...
      <div className="grid grid-cols-4 grid-rows-2 gap-6 h-[720px]">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-col gap-1 w-full mt-1">
                    <p className="text-xs text-gray-500 truncate">
                      <span className="font-bold text-gray-700">최근:</span>{" "}
                      {student.recentTask}
                    </p>
                    <div className="flex items-center gap-2 text-[10px] text-gray-400">
                      <Clock size={10} /> {student.lastLogin} 접속
                      <span className="text-indigo-600 font-black ml-auto bg-indigo-50 px-1.5 py-0.5 rounded">
                        D-{student.dDay}
                      </span>
                    </div>
                  </div>
        ...
        <div className="col-span-2 row-span-1 bg-white rounded-3xl border border-gray-100 shadow-sm p-6 flex flex-col hover:shadow-md transition-shadow">
          ...
          <div className="space-y-3 flex-1">
            {MENTOR_TASKS.filter((t) => t.status === "submitted")
              .slice(0, 2)
              .map((task, idx) => (
                <div
                  key={idx}
                  className="bg-gray-50 p-4 rounded-2xl flex items-start gap-4"
                >
                  ...
                  <button className="p-1 hover:bg-gray-100 rounded-lg text-gray-400 hover:text-gray-900 transition-colors flex-shrink-0">
