...
import {
  listTaskFeedbackByTaskId,
  markTaskFeedbackAsReadByTaskId,
} from "@/repositories/taskFeedbackRepository";
...
export async function markTaskFeedbackReadForMentee(
  taskId: string,
  menteeId: string,
) {
  const profile = await getProfileById(menteeId);
  ...
  return markTaskFeedbackAsReadByTaskId(taskId);
}
